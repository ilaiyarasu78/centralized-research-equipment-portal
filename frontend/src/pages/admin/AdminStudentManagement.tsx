...
export const AdminStudentManagement: React.FC = () => {
  ...
  useEffect(() => {
    fetchStudents();
  }, [search, departmentFilter, yearFilter, sectionFilter, statusFilter]);
  ...
  const handleDeleteStudent = async (id: string, name: string) => {
    if (!window.confirm(`Are you absolutely sure you want to permanently delete student "${name}" and all their associated bookings, reports, and profile details? This action is irreversible.`)) {
      return;
    }
    try {
  ...
  const handleStatusChange = async (newStatus: 'ACTIVE' | 'INACTIVE' | 'SUSPENDED') => {
    ...
    try {
      const res = await api.patch(`/admin/students/${selectedStudent.id}/status`, { status: newStatus });
      if (res.data.success) {
  ...
  const formatDate = (dateStr?: string | Date) => {
    if (!dateStr) return 'Never / Not Logged In';
  ...
  return (
    <div className="p-6 space-y-6 max-w-[1600px] mx-auto text-slate-900 bg-slate-50 min-h-screen">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <div className="flex items-center gap-2">
            <span className="px-2.5 py-1 rounded-md bg-purple-100 text-purple-900 font-extrabold text-[10px] uppercase tracking-wider border border-purple-300">
              SINGLE SOURCE OF TRUTH
            </span>
            <span className="text-xs font-bold text-slate-500">Master DB Sync</span>
          </div>
          ...
          <p className="text-xs text-slate-600 font-bold">
            Real-time registered student directory & comprehensive academic account administration
          </p>
        </div>

        <button
          onClick={fetchStudents}
          className="px-4 py-2 bg-white border border-slate-200 hover:bg-slate-100 text-slate-800 text-xs font-black rounded-2xl flex items-center gap-2 shadow-sm transition-all cursor-pointer shrink-0"
        >
        ...
        <div className="grid grid-cols-1 md:grid-cols-12 gap-3">
          ...
          <div className="md:col-span-4 relative">
            ...
            <input
              ...
              className="w-full pl-10 pr-4 py-2.5 bg-slate-50 border border-slate-200 rounded-2xl text-xs font-bold text-slate-900 placeholder-slate-400 focus:outline-none focus:border-purple-600 focus:bg-white transition-all"
            />
          </div>
          ...
          <div className="md:col-span-2">
            <select
              ...
              className="w-full px-3.5 py-2.5 bg-slate-50 border border-slate-200 rounded-2xl text-xs font-bold text-slate-900 focus:outline-none focus:border-purple-600 cursor-pointer"
            >
          ...
          <div className="md:col-span-2">
            <select
              ...
              className="w-full px-3.5 py-2.5 bg-slate-50 border border-slate-200 rounded-2xl text-xs font-bold text-slate-900 focus:outline-none focus:border-purple-600 cursor-pointer uppercase"
            >
      ...
      <div className="bg-white rounded-3xl border border-slate-200 shadow-sm overflow-hidden">
        <div className="p-4 border-b border-slate-100 flex items-center justify-between">
          <span className="text-xs font-extrabold text-slate-600 uppercase tracking-wider">
            Total Students Found: <strong className="text-purple-800 font-black">{students.length}</strong>
          </span>
          <span className="text-[11px] text-slate-500 font-medium">Automatic DB Retrieval</span>
        </div>
        ...
        {loading ? (
          <div className="p-12 text-center text-xs font-bold text-slate-500 space-y-2">
            ...
            <p>Fetching real-time student directory from PostgreSQL database...</p>
          </div>
        ) : students.length === 0 ? (
                      ...
                      <td className="p-4 text-center">
                        <div className="flex items-center justify-center gap-2">
                          <button
                            ...
                            className="px-3.5 py-1.5 bg-purple-600 hover:bg-purple-700 text-white font-black text-xs rounded-xl shadow-xs transition-all flex items-center gap-1.5 cursor-pointer"
                          >
                            <Eye className="w-3.5 h-3.5" /> View
                          </button>
                          <button
                            onClick={() => handleDeleteStudent(st.id, st.name)}
                            className="px-3.5 py-1.5 bg-red-50 hover:bg-red-100 text-red-600 hover:text-red-700 rounded-xl border border-red-200 font-black text-xs transition-all flex items-center gap-1.5 cursor-pointer"
                          >
      ...
      {/* STUDENT DETAILS MODAL / VIEW (/admin/students/:studentId) */}
      {selectedStudent && (
          ...
          <form onSubmit={handleSaveDetails} className="bg-white border border-slate-200 rounded-3xl w-full max-w-4xl max-h-[90vh] overflow-y-auto p-6 sm:p-8 space-y-6 shadow-2xl text-slate-900 relative">
            ...
            <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 pb-4 border-b border-slate-100">
              <div className="flex items-center gap-4">
                <div className="w-16 h-16 rounded-2xl bg-purple-600 text-white text-2xl font-black flex items-center justify-center shadow-md shrink-0">
                  ...
                  <div className="flex items-center gap-2">
                    {isEditing ? (
                      <input
                        ...
                        className="px-3 py-1.5 bg-slate-50 border border-slate-300 rounded-xl text-sm font-bold text-slate-900 focus:outline-none focus:border-purple-600"
                        required
                    ...
                    <span className="px-2.5 py-0.5 rounded-full text-[10px] font-black bg-purple-100 text-purple-900 border border-purple-300 uppercase">
                      STUDENT
                    </span>
                  </div>
                  <p className="text-xs text-purple-800 font-mono font-bold mt-0.5">
              ...
              <div className="flex flex-col gap-2 items-end shrink-0">
                {/* Account Status Changer Controls */}
                <div className="p-3 bg-slate-50 border border-slate-200 rounded-2xl space-y-1.5">
                  ...
                  <div className="flex items-center gap-1">
                    <button
                      ...
                      className={`px-2.5 py-1 text-[11px] font-black rounded-lg transition-all cursor-pointer ${
                        selectedStudent.accountStatus === 'ACTIVE'
                          ...
                          : 'bg-slate-200 text-slate-700 hover:bg-emerald-100 hover:text-emerald-900'
                      }`}
                    >
                      ACTIVE
                    </button>
                    <button
                      ...
                      className={`px-2.5 py-1 text-[11px] font-black rounded-lg transition-all cursor-pointer ${
                        selectedStudent.accountStatus === 'INACTIVE'
                    ...
                    <button
                      ...
                      className={`px-2.5 py-1 text-[11px] font-black rounded-lg transition-all cursor-pointer ${
                        selectedStudent.accountStatus === 'SUSPENDED'
                          ? 'bg-red-600 text-white shadow-xs'
                          : 'bg-slate-200 text-slate-700 hover:bg-red-100 hover:text-red-900'
                      }`}
                    >
                      SUSPENDED
                    </button>
                ...
                {!isEditing ? (
                  <button
                    ...
                    className="w-full px-4 py-2 bg-purple-600 hover:bg-purple-700 text-white font-black text-xs rounded-xl shadow-md cursor-pointer flex items-center justify-center gap-1.5"
                  >
            ...
            {/* GRID: PERSONAL, ACADEMIC, ACCOUNT INFO */}
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              {/* Personal Information Card */}
              <div className="p-4 rounded-2xl bg-purple-50/50 border border-purple-200 space-y-2">
                <h3 className="text-xs font-black text-purple-900 uppercase tracking-wide flex items-center gap-1.5 border-b border-purple-200 pb-2">
                  <Mail className="w-3.5 h-3.5 text-purple-600" /> Personal Information
                </h3>
                  ...
                  <div>
                    ...
                    {isEditing ? (
                      <input
                        ...
                        className="w-full px-2 py-1 bg-white border border-slate-300 rounded-lg text-xs font-bold text-slate-900 focus:outline-none focus:border-purple-600"
                        required
                      />
                    ) : (
              ...
              <div className="p-4 rounded-2xl bg-blue-50/50 border border-blue-200 space-y-2">
                <h3 className="text-xs font-black text-blue-900 uppercase tracking-wide flex items-center gap-1.5 border-b border-blue-200 pb-2">
                  <GraduationCap className="w-3.5 h-3.5 text-blue-600" /> Academic Information
                </h3>
                    ...
                    {isEditing ? (
                      <div className="flex gap-1">
                        <select
                          ...
                          className="w-1/2 px-1 py-1 bg-white border border-slate-300 rounded-lg text-xs font-bold text-slate-900 focus:outline-none focus:border-purple-600"
                        >
              ...
              <div className="p-4 rounded-2xl bg-amber-50/50 border border-amber-200 space-y-2">
                ...
                <div className="space-y-1.5 text-xs">
                  <div>
                    ...
                    {isEditing ? (
                      <input
                        ...
                        className="w-full px-2 py-1 bg-white border border-slate-300 rounded-lg text-xs font-bold text-slate-900 focus:outline-none focus:border-purple-600 font-mono"
                        required
                      />
                    ) : (
                  ...
                  {!isEditing && (
                    <>
                      <div>
                        <span className="text-[10px] text-slate-500 font-bold block">Registration Date</span>
                        <strong className="text-slate-900">{formatDate(selectedStudent.createdAt)}</strong>
                      </div>
                      <div>
                        <span className="text-[10px] text-slate-500 font-bold block">Last Login</span>
                        <strong className="text-purple-800">{formatDate(selectedStudent.lastLoginAt)}</strong>
                      </div>
                    </>
                  )}
                </div>
            ...
            {/* PORTAL ACTIVITY (Bookings, Requests, Issues) */}
            <div className="space-y-4 pt-2">
              <h3 className="text-xs font-black text-slate-900 uppercase tracking-wider flex items-center gap-2">
                <Boxes className="w-4 h-4 text-purple-600" />
                Smart Campus Portal Activity Log
              </h3>
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                ...
                <div className="p-4 rounded-2xl bg-slate-50 border border-slate-200 space-y-2">
                  <div className="flex items-center justify-between">
                    <span className="text-xs font-bold text-slate-900 flex items-center gap-1.5">
                      <Wrench className="w-3.5 h-3.5 text-purple-600" /> Lab & Equipment Bookings
                    </span>
                  ...
                  {selectedStudent.bookings?.length === 0 ? (
                    <p className="text-[11px] text-slate-500">No equipment bookings made yet.</p>
                  ) : (
                    <div className="space-y-1.5 max-h-36 overflow-y-auto pr-1">
                      {selectedStudent.bookings?.map((b: any) => (
                        <div key={b.id} className="p-2 rounded bg-white border border-slate-200 text-[11px] space-y-0.5">
                          ...
                          <span className="text-[9px] font-black text-purple-700">{b.status}</span>
                        </div>
                      ))}
                    </div>
                  )}
                ...
                <div className="p-4 rounded-2xl bg-slate-50 border border-slate-200 space-y-2">
                  <div className="flex items-center justify-between">
                    <span className="text-xs font-bold text-slate-900 flex items-center gap-1.5">
                      <BookOpen className="w-3.5 h-3.5 text-blue-600" /> Equipment Requests
                    </span>
                  ...
                  {selectedStudent.requests?.length === 0 ? (
                    <p className="text-[11px] text-slate-500">No equipment requests submitted.</p>
                  ) : (
                    <div className="space-y-1.5 max-h-36 overflow-y-auto pr-1">
                      {selectedStudent.requests?.map((r: any) => (
                        <div key={r.id} className="p-2 rounded bg-white border border-slate-200 text-[11px] space-y-0.5">
                          ...
                          <span className="text-[9px] font-black text-blue-700">{r.status}</span>
                        </div>
                      ))}
                    </div>
                  )}
                ...
                <div className="p-4 rounded-2xl bg-slate-50 border border-slate-200 space-y-2">
                  ...
                  {selectedStudent.issues?.length === 0 ? (
                    <p className="text-[11px] text-slate-500">No tickets or issue reports submitted.</p>
                  ) : (
                    <div className="space-y-1.5 max-h-36 overflow-y-auto pr-1">
            ...
            <div className="pt-4 border-t border-slate-100 flex justify-between items-center">
              <button
                ...
                className="px-5 py-2.5 bg-red-600 hover:bg-red-500 text-white font-black text-xs rounded-xl shadow-md cursor-pointer flex items-center gap-1.5"
              >
                <UserX className="w-4 h-4" /> Delete Student Account
              </button>
              <button
                type="button"
                onClick={() => navigate('/admin/students')}
