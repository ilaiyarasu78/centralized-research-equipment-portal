    ...
    <aside className="w-64 bg-white border-r border-slate-200 min-h-screen flex flex-col justify-between p-4 select-none shrink-0 z-20 transition-colors shadow-sm">
      <div>
        ...
        <div className="space-y-1">
          <NavLink
            to={user?.role === 'ADMIN' ? '/admin/dashboard' : user?.role === 'STAFF' ? '/staff/dashboard' : '/student/dashboard'}
            className={({ isActive }) =>
          ...
          <NavLink
            to={user?.role === 'ADMIN' ? '/admin/notifications' : user?.role === 'STAFF' ? '/staff/notifications' : '/student/notifications'}
            className={({ isActive }) =>
                    ...
                    <NavLink
                      ...
                      className={({ isActive }) =>
                        `flex items-center gap-2 px-3 py-2 rounded-lg text-xs font-semibold transition-all ${
                          isActive ? 'bg-purple-100 text-purple-950 border border-purple-300 font-extrabold shadow-xs' : 'text-slate-700 hover:text-purple-700 hover:bg-purple-50'
                        }`
                      }
                    >
      ...
      <div className="space-y-4 pt-4 border-t border-slate-200">
        ...
        {user?.role === 'STUDENT' && (
          <div className="p-3 rounded-2xl bg-gradient-to-br from-purple-50 to-indigo-50 border border-purple-200 relative overflow-hidden shadow-xs">
            <div className="relative z-10 flex items-start gap-2.5">
              ...
              <div>
                ...
                <button
                  ...
                  className="w-full py-1.5 px-3 rounded-xl bg-purple-100 hover:bg-purple-200 text-purple-950 border border-purple-300 text-[11px] font-black transition-all shadow-xs cursor-pointer"
                >
                  Contact Now
                </button>
              </div>
